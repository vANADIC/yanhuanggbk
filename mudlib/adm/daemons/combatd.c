...
STATIC_VAR_TAG string foo_before_hit = 0;
STATIC_VAR_TAG string foo_after_hit = 0;
...
string damage_msg(int damage, string type)
{
        ...
        case "枪伤":
                if (damage < 10)  return HIC "结果子弹只是轻轻地擦过，并没有真正击中$n" HIC "。\n" NOR; else
                if (damage < 100) return HIR "结果$N" HIR "这一枪正中$n" HIR "的$l，直把$n" HIR "轰得血肉模糊！\n" NOR; else
...
void fight(object me, object victim)
{
	...
	} else if (to_int(me->query_str()) > random(to_int(victim->query_str() * 3 / 4 )))
        {
                me->set_temp("guarding", 0);
...
int player_escape(object killer, object ob)
{
        ...
        {
                msg = HIW "此时正逢一男子路过，见了" + ob->name() + HIW
                      "晕倒在地，咦了一声，随即将他背负在背上，跨步远去"
                      "。\n" NOR;
...
void killer_reward(object killer, object victim)
{
        ...
        if (playerp(victim))
        {
                   ...
                   && victim->query_temp("guard_death") < 1)
                {
                       ...
                       //生死玄关 by 薪有所属
                       if (! victim->query("death")) flag=100;//没打通生死玄关时掉技能几率为百分之百
                        else {
                                 ...
                                 */
                                 ...
                                 if (victim->query("combat_exp")>= 30000000) flag=0;
                                 	else if (victim->query("combat_exp")>= 25000000) flag=10;
                                		else if (victim->query("combat_exp")>= 20000000) flag=20;
                                			else if (victim->query("combat_exp")>= 15000000) flag=30;
                                				else if (victim->query("combat_exp")>= 10000000) flag=40;
                                					else if (victim->query("combat_exp")>= 5000000) flag=50;
                                 else flag = 60;
                        }
